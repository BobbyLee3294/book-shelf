  ...
  async function searchBooks(event) {
    ...
    try {
      const response = await axios.get(OPEN_LIBRARY_URL, {
        params: { q: query, limit: 12, fields: "key,title,author_name,cover_i,first_publish_year" },
      });
    ...
    <main className="search-page">
      ...
      <label className="shelf-picker">
        ...
        <select value={selectedShelf} onChange={(event) => setSelectedShelf(event.target.value)}>
          {bookshelves.map((shelf) => <option key={shelf.id} value={shelf.id}>{shelf.name}</option>)}
        </select>
      </label>
      ...
      <section className="book-results" aria-live="polite">
        {results.map((book) => (
          <article className="book-result" key={book.key}>
            {book.cover_i ? <img src={`https://covers.openlibrary.org/b/id/${book.cover_i}-M.jpg`} alt="" /> : <div className="cover-placeholder">No cover</div>}
            <div>
              <h2>{book.title}</h2>
              <p>{book.author_name?.[0] || "Unknown author"}</p>
              {book.first_publish_year && <small>First published {book.first_publish_year}</small>}
              <button type="button" onClick={() => saveBook(book)} disabled={!selectedShelf}>Add to shelf</button>
            </div>
          </article>
        ))}
      </section>
